        ...
        <table class="table table-bordered">
          ...
          <tbody class="tbody">
            <tr v-for="report in reports" :key="report._id">
              ...
              <td>{{ report.type }}</td>
              <td>
                {{ report.student_id.name + ' ' + report.student_id.last_name }}
              </td>
              <td>{{ report.description }}</td>
              <td>
                <button class="btn edit" @click="reportDetails(report._id)">
      ...
      <div v-if="!loading" class="pagination-container">
        ...
        <div class="arrows-container">
          <span
            ...
            {{
              pageResults > totalReports
                ? totalReports
                : page * pageResults > totalReports
                ? totalReports
                : page * pageResults
            }}
            de {{ totalReports }} reportes</span
          >
          <button
...
export default {
  ...
  methods: {
    ...
    before() {
      if (this.page == 1) {
        this.disbaledBefore = 1;
        if (this.totalReports == 0) this.disabledAfter = 1;
        else {
          if (this.totalReports > this.pageResults) this.disabledAfter = 0;
