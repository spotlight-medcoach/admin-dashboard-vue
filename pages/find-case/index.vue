  ...
  <div>
    ...
    <div class="case-container">
      <div class="title">
        ...
        <button v-if="!loading" class="btn" @click="addNewCase">
          <i class="fas fa-list-alt"></i> Crear un nuevo caso
        </button>
      </div>
      ...
      <div v-if="!loading" class="filter-container">
        ...
        <select
          v-model="subtopicBubbleSelected"
          name="subtopic"
          class="subtopic js-example-basic-single"
          @change="getCasesFilteredBySubtopic(subtopicBubbleSelected)"
        >
            ...
            <tr v-for="theCase in cases" :key="theCase._id">
              ...
              <td class="td-actions">
                <label class="switch">
                  <input
                    type="checkbox"
                    v-model="theCase.active"
                    @change="changeStatusCase(theCase)"
                  />
                  <span class="slider round"></span>
                </label>
                <button
                  class="pencil btn fas fa-pencil-alt"
                  @click="caseDetails(theCase._id)"
                ></button>
                <button
                  class="plus btn fas fa-plus-square"
                  @click="addToSimulatorConfirm(theCase._id)"
                ></button>
                <button
                  class="trash btn fas fa-trash"
                  @click="deleteConfirm(theCase._id)"
                ></button>
              </td>
            </tr>
          </tbody>
...
export default {
  ...
  methods: {
    ...
    async getCases() {
      try {
        ...
        this.cases.forEach((theCase) => {
          ...
          theCase.name_subtopic = this.filterSubtopicName(
            theCase.topic,
            theCase.subtopic
          );
        });
    ...
    addNewCase() {
      this.$router.push({
        path: `/find-caseaddNewCase`,
        query: { length: this.cases.length },
      });
    },
    ...
    async addToSimulator() {
      try {
        ...
        this.bodyModal =
          'El caso se agregó correctamente al "' +
          this.simulators.filter((sim) => sim._id == this.simulatorSelected)[0]
            .name +
          '". Para verlo, ve al simulador y búscalo en el tema al que pertenece.';
        this.button = 'Entendido';
        this.isShowInfoModal = !this.isShowInfoModal;

        this.cases = this.cases.filter(
          (eachCase) => eachCase._id != this.caseToAddASimulator
        );
      } catch (err) {
        console.log(err);
        this.busyAddToSimulator = !this.busyAddToSimulator;
    ...
    caseDetails(theCase) {
      this.$router.push({ path: `/find-case${theCase}` });
    },
    deleteConfirm(theCase) {
      this.titleModal = 'Eliminar caso';
      this.bodyModal =
        '¿Deseas eliminar este caso? Esta acción no puede deshacerse.';
    ...
    async deleteCase() {
      try {
        ...
        setTimeout(() => {
          this.showSuccessToast = !this.showSuccessToast;
          this.cases = this.cases.filter(
            (theCase) => theCase._id != deleteResponse.data.payload
          );
          this.totalCases -= 1;
