  ...
  <div>
    ...
    <div v-else class="details-container">
      ...
      <div class="cancel-container">
        ...
        >
          <i class="fas fa-trash mr-1"></i> Descartar caso
        </button>
      </div>
      ...
      <div class="questions-container">
        ...
        <div
          v-if="caseDetails.status != 'Approved'"
          class="add-question-container"
        >
          <button class="btn" @click="requestQuestion">
      ...
      <div v-if="caseDetails.status != 'Approved'" class="buttons-container">
        <button class="btn draft" @click="draftConfirm">
          <i class="fas fa-save"></i> Guardar como borrador
        </button>
        <button class="btn send" @click="saveAndSendConfirm">
          <i class="fas fa-paper-plane"></i> Guardar y enviar caso
        </button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getCaseDetails() {
      ...
      this.caseDetails.name_topic = this.filterTopic(
        this.caseDetails.topic_bubble
      );
      this.caseDetails.name_subtopic = this.filterSubtopic(
        this.caseDetails.topic_bubble,
      ...
      this.contentDescription =
        this.caseDetails.description.content.ops[0].insert;
        ...
        if (arrayName.length >= 4) {
          ...
          if (arrayName[1] === 'undefined') {
            console.log(
              'Folio:',
              this.topics.find(
                (top) => top.bubble_id == this.caseDetails.topic_bubble
              ).topic_folio
            );
            arrayName[1] =
              this.topics.find(
                (top) => top.bubble_id == this.caseDetails.topic_bubble
              ).topic_folio ?? arrayName[1];
          }
          if (arrayName[2] === 'undefined') {
            console.log(
              'Folio:',
              this.topics
                .find((top) => top.bubble_id == this.caseDetails.topic_bubble)
                .subtopics.find(
                  (sub) => sub.subtopic == this.caseDetails.subtopic_bubble
                ).subtopic_folio
            );
            arrayName[2] =
              this.topics
    ...
    async addQuestion() {
      try {
        ...
        let questionsResponse = await this.$axios.post(
    ...
    viewQuestion(question) {
      ...
      this.questionAcceptedToView.type_name = this.types.filter(
    ...
    draftConfirm() {
      ...
      this.bodyModal =
        'Tu caso estará en estado de edición, solo tú lo podrás ver para que puedas continuar editandolo.';
    ...
    saveAndSendConfirm() {
      if (this.contentHtml.trim() == '' || this.indexQuestion == 0) {
        this.titleModal =
          'Todos los campos deben ser llenados o falta agregar preguntas';
        this.showFailToast = !this.showFailToast;
        ...
        this.bodyModal =
          'Tu caso se enviará al panel de administración para ser revísado. Si toda la información es correcta, se publicará en los simuladores o en caso de ser necesario, recibirás comentarios y feedback para su corrección. \n Una vez enviado no podrás modificarlo a menos que lo solicite el administrador. ¿Deseas enviarlo?';
    ...
    async discardCase() {
      try {
        ...
        let deleteCaseResponse = await this.$axios.delete(
          '/deletePendingCase',
          {
            params: { case_id: this.$route.params.id },
          }
