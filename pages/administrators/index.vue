    ...
    <div class="admin-container">
      ...
      <div v-else class="search-active-container">
        <input
          ...
          type="searchText"
          placeholder="Buscar"
          @keyup.enter="searchAdministrators"
        />

        <select
          v-model="selected"
          class="options"
          @change="selectedChange(selected)"
        >
                  ...
                  >
                    <div class="configuration">
                      <button
                        ...
                        type="button"
                        class="btn"
                        @click="update(administrators[index])"
                      >
                      ...
                      <button
                        ...
                        type="button"
                        class="btn"
                        @click="confirmModalActive(administrators[index])"
                      >
                    ...
                    <div v-if="selected == 'true'" class="notifications">
                      <button
                        type="button"
                        class="btn"
                        @click="confirmModalInactive(administrators[index])"
                      >
      ...
      <div v-if="!loading" class="pagination-container">
        ...
        <div class="arrows-container">
          <span
            ...
            {{
              pageResults > totalAdmins
                ? totalAdmins
                : page * pageResults > totalAdmins
                ? totalAdmins
                : page * pageResults
            }}
            de {{ totalAdmins }} administradores</span
          >
          <button
...
export default {
  ...
  async created() {
    ...
    this.administrators =
      this.$store.getters['administrators/getAdministrators'];
    this.totalAdmins =
  ...
  methods: {
    ...
    async activateUser() {
      try {
        ...
        let userActivated = await this.$store.dispatch(
          'administrators/activateUser',
          this.userIdToActive
        );
    ...
    async inactivateUser() {
      try {
        ...
        let userInactivated = await this.$store.dispatch(
    ...
    before() {
      if (this.page == 1) {
        this.disbaledBefore = 1;
        if (this.totalAdmins == 0) this.disabledAfter = 1;
        else {
          if (this.totalAdmins > this.pageResults) this.disabledAfter = 0;
