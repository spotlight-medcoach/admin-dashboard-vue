<template>
  <div id="diagnostic-question-edit">
    <section class="question-form-container">
      <article class="full">
        <loading-state
          v-if="loadingState"
          message="Estamos obteniendo los datos de la pregunta"
        />
        <b-form
...
export default {
  ...
  async mounted() {
    this.$store.dispatch('pageHeader/setHeader', {
      title: 'Editar Pregunta de Diagnóstico',
      buttonConfig: {
        type: 'button',
        isBack: true,
    ...
    if (questionId) {
      await this.$store.dispatch(
        'diagnosticQuestions/fetchQuestionById',
        questionId
      );
  ...
  methods: {
    ...
    loadQuestionData() {
      if (this.currentQuestion && this.topics && this.topics.length > 0) {
        ...
        this.form.question_html = this.currentQuestion.question?.html || '';
        this.form.retro_html = this.currentQuestion.retro?.html || '';

        // Load answers
        this.form.answers = (this.currentQuestion.answers || []).map(
          (answer, index) => ({
            id: answer.id || index,
            html: answer.html || '',
            is_correct: index === this.currentQuestion.correct_answer,
          })
        );
      }
    },
    ...
    async updateQuestion() {
      try {
        ...
        await this.$store.dispatch('diagnosticQuestions/updateQuestion', {
