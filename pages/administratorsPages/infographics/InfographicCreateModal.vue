...
export default {
	...
	computed: {
		topicOptions () {
			const initial = [{ text: '-- Tema de infografía', value: null, disabled: true }]
			return initial.concat(
				this.topics
					.map((topic) => { return { text: topic.name, value: topic } })
			)
		},
		subtopicOptions () {
			const initial = [{ text: '-- Subtema de infografía', value: null, disabled: true }]
			if (this.form.topic === null) {
				return initial
			}
			return initial.concat(
				this.form.topic.subtopics
					.map((subtopic) => { return { text: subtopic.name, value: subtopic } })
			)
		}
	},
	watch: {
