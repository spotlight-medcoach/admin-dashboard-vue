<template>
  <div>
    ...
    <div class="details-container">
      <div class="back-btn">
        <nuxt-link to="/review-new-questions">
      ...
      <div class="title-container">
        ...
        <div v-if="!loading" class="buttons">
          <button type="button" class="btn cancel" @click="discardConfirm">
            <i class="fas fa-trash"></i> Descartar caso
          </button>

          <button
            v-if="caseDetails.status != 'With feedback'"
            type="button"
            class="btn retro-btn"
            @click="retroAlert"
          >
          ...
          <button v-else class="btn retro-btn" @click="viewRetro">
      ...
      <div v-if="!loading" class="buttons-container">
        <button class="btn bank" @click="addToBankConfirm">
          <i class="fas fa-save"></i> Agregar al banco
        </button>
        <button class="btn send" @click="addToSimulatorConfirm">
          <i class="fas fa-paper-plane"></i> Agregar al simulador
        </button>
      </div>
    </div>
  ...
  methods: {
    ...
    async getCaseDetails() {
      try {
        ...
        let caseResponse = await this.$axios.get('/getDetails', {
    ...
    updateFeedback() {
      try {
        ...
        setTimeout(() => {
          this.loading = !this.loading;
          this.showSuccessToast = !this.showSuccessToast;
          this.$router.push({ path: `/review-new-questions` });
        }, 1000);
      } catch (err) {
    ...
    discardConfirm() {
      ...
      this.bodyModal =
        'Al descartar caso, únicamente se eliminará la información del usuario actual y podrá volver a ser asignado a alguien más.';
