        ...
        <table class="table table-bordered">
          ...
          <tbody class="tbody">
            <tr v-for="(theCase, index) in cases" :key="theCase._id">
              ...
              <td>
                {{
                  theCase.approved_date
                    ? new Date(theCase.approved_date).toLocaleDateString(
                        'es-ES'
                      )
              ...
              <td>
                <button class="btn options" @click="caseDetails(cases[index])">
...
export default {
  ...
  methods: {
    async getCasesWithReview() {
      try {
        ...
        cases.forEach((oneCase) => {
          ...
          oneCase.subtopic_name = this.getSubtopicName(
    ...
    caseDetails(theCase) {
      ...
      this.$router.push({
        path: `/review-new-questions/${theCase._id}?totalCases=${this.cases.length}`,
      });
    },
    rowsChange() {
      this.page = 1;
