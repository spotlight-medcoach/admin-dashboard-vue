    ...
    <div class="add-case-container">
      ...
      <div v-else class="form-container">
        <div class="title-container">
          ...
          <button type="button" class="btn cancel" @click="discardCaseConfirm">
        ...
        <div class="assign">
          ...
          <div class="input">
            <select
              v-model="spotlighterSelected"
              class="js-example-basic-single"
              name="state"
              @change="setSelectedUser(spotlighterSelected)"
            >
              <option :value="''" selected>No asignar a alguien</option>
              <option
                :value="spot.spotlighter_id"
                v-for="spot in spotlighters"
                :key="spot.spotlighter_id"
              >
                {{ spot.name }} {{ spot.last_name }}
              </option>
            </select>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async getSpotlighters() {
      try {
        let spotlighters_response = await this.$axios.get(
      ...
      try {
        ...
        if (
          ...
          let case_response = await this.$axios.post('/createPendingCase', {
            ...
            status: this.spotlighterSelected
              ? 'Accepted by Spotlighter'
              : 'Pending',
          });
    ...
    discardCaseConfirm() {
      ...
      this.textModal =
        'Al descartar esete caso, se eliminará toda la información relacionada con él. Esta acción no puede deshacerse. ¿Deseas descartarlo?';
