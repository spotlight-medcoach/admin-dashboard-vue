    ...
    <div class="cases-container">
      <div class="head-container">
        ...
        <nuxt-link
          v-if="!loading"
          :to="{
            path: '/requested-cases/add-requested-case',
            query: {
              length: cases.filter((cas) => cas.requested == true).length,
            },
          }"
      ...
      <div v-else class="search-container">
        ...
        >
          ...
          <option value="Accepted by Spotlighter">
            Aceptado por spotlighter
          </option>
          ...
          <tbody class="tbody">
            <tr v-for="theCase in cases" :key="theCase._id">
              ...
              <td
                ...
                :title="theCase.description.content.ops[0].insert"
                class="over"
              >
                {{ theCase.description.content.ops[0].insert }}
              </td>

              <td v-if="theCase.status == 'Pending'">
                <div class="pending">Pendiente</div>
              </td>
              <td v-else-if="theCase.status == 'Accepted by Spotlighter'">
                <div class="accepted">Aceptado</div>
              </td>
              <td v-else-if="theCase.status == 'In edit'">
              ...
              <td v-else-if="theCase.status == 'Pending review'">
                <div class="pendign-review">Revisión pendiente</div>
              </td>

              <td>
                {{
                  theCase.spotlighter_id
                    ? theCase.spotlighter_id.admin_id.name +
                      ' ' +
                      theCase.spotlighter_id.admin_id.last_name
                    : 'No asignado/aceptado'
                }}
              </td>
              <td class="act">
                <div
                  v-if="
                    theCase.status == 'Accepted by Spotlighter' ||
                    theCase.status == 'Pending' ||
                    theCase.status == 'In edit'
                  "
                  class="appro"
                >
                  <button
                    class="btn accep"
                    @click="acceptedCaseModal(theCase.status)"
                  >
                    <i class="fas fa-exclamation-circle"></i> Info
                  </button>
                </div>
                <div v-else class="op">
                  <button class="btn op" @click="caseDetails(theCase._id)">
...
export default {
  ...
  methods: {
    async getCases() {
      ...
      try {
        ...
        cases.forEach((oneCase) => {
          ...
          oneCase.subtopic_name = this.filterSubtopic(
            oneCase.topic_bubble,
            oneCase.subtopic_bubble
          );
        });
    ...
    caseDetails(case_id) {
      ...
      this.$router.push({ path: `/requested-cases/${case_id}` });
    },
    acceptedCaseModal(status) {
      ...
      if (status == 'Pending') {
        ...
        this.bodyModal =
          'Este caso no ha sido asignido y aún no es aceptado por algún spotlighter.';
        this.button = 'Entendido';
      } else if (status == 'Accepted by Spotlighter') {
        this.titleModal = 'Caso aceptado por spotlighter';
        this.bodyModal =
          'Cuando un caso ha sido aceptado no podrás ver su detalle hasta que el usuario lo mande a revisión.';
        this.button = 'Entendido';
      } else {
        this.titleModal = 'Caso en edición';
        this.bodyModal =
          'Este caso esta en edición, podrás ver su contenido hasta que el spotlighter lo mande para revisión.';
        this.button = 'Entendido';
      }
