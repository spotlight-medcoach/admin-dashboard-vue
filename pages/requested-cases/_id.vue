  ...
  <div>
    ...
    <div class="details-container">
        ...
        <div v-if="!loading" class="buttons">
          <div v-if="caseDetails.case_status == 'Pending'">
            <button
              type="button"
              class="btn cancel"
              @click="discardCaseConfirm"
            >
              <i class="fas fa-trash px-2"></i>Eliminar caso
            </button>
          </div>

          <div v-else-if="caseDetails.case_status == 'Pending review'">
            <button type="button" class="btn retro-btn" @click="retroAlert">
              <i class="fas fa-exclamation"></i> Dar retroalimentación
            </button>
          </div>

          <div v-else-if="caseDetails.case_status == 'With feedback'">
            <button type="button" class="btn retro-btn" @click="viewRetro">
              <i class="fas fa-exclamation"></i> Ver retroalimentación
            </button>
          </div>
        </div>
      </div>
      ...
      <CaseDetailsReviewCard
        ...
        :request_description="
          caseDetails.request_description.content.ops[0].insert
        "
        :topic="caseDetails.name_topic"
        :subtopic="caseDetails.name_subtopic"
        :user="
          caseDetails.admin_user.name + ' ' + caseDetails.admin_user.last_name
        "
      />

      <div v-if="!loading" class="description-container">
        <h1>Descripción del caso</h1>
        <span v-if="caseDetails.status == 'Pending'" class="my-3 text-danger"
          >Aún no hay información sobre este caso.</span
        >
        <quill-editor
          v-else
          disabled
      ...
      <div v-if="!loading" class="questions-container">
        ...
        <span v-if="caseDetails.status == 'Pending'" class="my-3 text-danger"
          >Aún no hay preguntas asignadas a este caso.</span
        >
      ...
      <div v-if="!loading" class="buttons-container">
        <div
          v-if="
            caseDetails.case_status == 'Pending review' ||
            caseDetails.case_status == 'With feedback'
          "
        >
          <button class="btn draft" @click="bankConfirm">
            <i class="fas fa-book"></i> Autorizar y agregar al banco
          </button>
          <button class="btn send" @click="simulatorConfirm">
            <i class="fas fa-book-open"></i> Autorizar y agregar al simulador
          </button>
        </div>
      </div>
    </div>
...
import QuestionReviewCard from '@/components/cards/administrators/question-review.card.vue';
import QuestionDetailsReviewModalAdministrator from '@/components/modals/administrators/question-details-review-administrator.modal.vue';
import CaseDetailsReviewCard from '@/components/cards/administrators/case-details-review.card.vue';
import SetRetroModal from '@/components/modals/administrators/set-retro.modal.vue';
import ViewRetroModal from '@/components/modals/administrators/view-retro.modal.vue';
import AddToBankModal from '@/components/modals/administrators/add-to-bank.modal.vue';
import AddToSimulator from '@/components/modals/administrators/add-to-simulator.modal.vue';
  ...
  methods: {
    ...
    async getCaseDetails() {
      try {
        ...
        let caseDetailsResponse = await this.$axios.get('/getDetails', {
